...
export const CompanyPortal: React.FC<CompanyPortalProps> = ({
  ...
  const [contratacoesList, setContratacoesList] = useState([
    {
      ...
      statusProcesso: 'Documentação Pendente',
      remuneration: 'Não informada',
      salaryAmount: 'R$ 6.500,00',
      honorariosAmount: 'R$ 13.000,00',
  ...
  const isModuleEnabled = (moduleId: string): boolean => {
    ...
    if (Array.isArray(currentCompany.modules) && currentCompany.modules.length > 0) {
      return currentCompany.modules.includes(moduleId);
    }
    return true; // Fallback to all enabled if company modules not yet configured
  };
    ...
    <div className="min-h-screen bg-[#070d18] text-slate-100 flex flex-col font-sans select-none">
                        ...
                        <div className="p-5 space-y-4">
                          ...
                          <div className="flex items-center justify-between pt-2">
                            ...
                            {c.sentToHeadhunter ? (
                              <button
                                onClick={() => setActiveMenu('headhunter')}
                                className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-xl shadow-md flex items-center space-x-1.5 transition transform active:scale-95"
                              >
                                <span>Ver no Módulo Headhunter →</span>
                              </button>
                            ) : (
                              <button
                                onClick={() => {
      ...
      {selectedForwardItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 font-sans select-none border border-slate-200 animate-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900">Encaminhar ao Headhunter & Financeiro</h3>
                  <p className="text-xs text-slate-500">Enviar contratação finalizada para faturamento de honorários</p>
                </div>
              </div>
              <button
                onClick={() => setSelectedForwardItem(null)}
            ...
            <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200/80 space-y-1.5 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-extrabold text-slate-900">{selectedForwardItem.candidateName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-bold">Cargo / Vaga:</span>
                <span className="font-extrabold text-blue-600">{selectedForwardItem.jobTitle}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-bold">Data de Fechamento:</span>
                <span className="font-extrabold text-slate-800">{selectedForwardItem.date}</span>
              </div>
            </div>
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleConfirmForwardToHeadhunter();
              }}
              className="space-y-3 text-xs"
            >
              <div>
                <label className="block font-bold text-slate-700 mb-1">Cliente Corporativo / Solicitante *</label>
                <input
                  type="text"
                  required
                  value={forwardForm.client}
                  onChange={(e) => setForwardForm({ ...forwardForm, client: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Salário / Remuneração *</label>
                  <input
                    type="text"
                    required
                    value={forwardForm.salary}
                    onChange={(e) => setForwardForm({ ...forwardForm, salary: e.target.value })}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Honorários Headhunter *</label>
                  <input
                    type="text"
                    required
                    value={forwardForm.fee}
                    onChange={(e) => setForwardForm({ ...forwardForm, fee: e.target.value })}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Observações de Faturamento</label>
                <textarea
                  rows={2}
              ...
              <div className="flex justify-end space-x-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white font-extrabold rounded-xl shadow-md transition flex items-center space-x-1.5"
                >
                  <DollarSign className="w-4 h-4" />
                  <span>Confirmar e Enviar ao Headhunter</span>
                </button>
              </div>
            </form>
          </div>
